...
class LessonModel {
  ...
  getLessonByTeacherId = async (teacher_id) => {
    var sql =
      "select Lesson.* from Lesson left join Teach on Teach.course_id = Lesson.course_id where teacher_id=" + teacher_id + ";";
  ...
  createLesson = async (course_id) => {
    ...
    var sql =
      "INSERT INTO Lesson (lesson_name, course_id, time_started, time_ended, lesson_date) value ('" +
      l.lesson_name +
      "', " +
      l.course_id +
      ", '" +
